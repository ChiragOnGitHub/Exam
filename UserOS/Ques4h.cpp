...
int main(int argc, char **argv){
    ...
    int permission=buffer.st_mode & 0700; // the least significant 9 bits of st_mode field contain the permission bits of the file.
    ...
    {cout << "Owner has read permision." << endl;} // So, octal bit mask of 0400(100 000 000) will retrieve owner read permission
    else
    {cout << "Owner does not have read permission." << endl;}
    if ((permission & 0200)==0200) // similarly, if the owner has write permisiion, then the bit pattern will have 1 in the 8th position.
    {cout << "Owner has write permission." << endl;}// 0200->010 000 000 (1 in the 8th position)
    else
    {cout << "Owner does not have write permission." << endl;}
    if ((permission & 0100)==0100) // same explanation for execute permission
    {cout << "Owner has execute permission." << endl;} // 0100->001 000 000 (1 in the 7th position)
    else
